//! Helper utilities for implement transports.
...
pub(super) async fn receive<R, M>(reader: &mut R) -> Option<(u64, Result<M>)>
where
...
pub(super) async fn send<W, M>(writer: &mut W, id: u64, message: M) -> bool
where
...
/// Helper for implementing servers that use underlying stream/sink of bytes.
pub(super) async fn handle_server_connection<T>(mut socket: T, server_state: &ServerState)
where
...
/// Helper for implementing clients that use underlying stream/sink of bytes.
pub(super) struct Invoker<R, W> {
    request_tx: mpsc::Sender<(Request, oneshot::Sender<Result<Response>>)>,
    _reader: PhantomData<R>,
    _writer: PhantomData<W>,
}

impl<R, W> Invoker<R, W>
where
    ...
    W: Sink<Bytes, Error = io::Error> + Unpin + Send + 'static,
{
    pub fn new(reader: R, writer: W) -> Self {
        ...
        task::spawn(InvokerWorker::new(request_rx, reader, writer).run());
