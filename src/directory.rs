...
impl Directory {
    ...
    pub async fn open(
        ...
        let mut blob = Blob::open(pool, secret_key, parent_name, seq).await?;
    ...
    pub fn create(
        ...
        let blob = Blob::create(pool, secret_key, parent_name, seq);
        ...
        match self.data.entry_type {
            EntryType::File => Ok(Entry::File(
                File::open(
                    ...
                    Some(*self.parent_blob.head_name()),
                    self.data.seq,
                )
                .await?,
            )),
