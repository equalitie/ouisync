...
pub(crate) async fn migrate_dirs() {
    let (report_tx, mut report_rx) = mpsc::channel(1);

    let work = async move {
        let new = config_dir();
        for old in old_config_dirs() {
            migrate_dir(&old, &new, ConflictStrategy::KeepDst, report_tx.clone()).await;
        }

        let new = store_dir();
        for old in old_store_dirs() {
            migrate_dir(&old, &new, ConflictStrategy::Rename, report_tx.clone()).await;
        }
