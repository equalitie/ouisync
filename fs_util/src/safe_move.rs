...
/// Otherwise it copies `src` to `dst` first and then deletes `src`. This function does not
/// overwrite `dst` if it already exists, instead it fails with `AlreadyExists` error.
...
fn blocking_safe_rename(src: &Path, dst: &Path) -> io::Result<()> {
    use std::{fs, iter, os::windows::ffi::OsStrExt};
...
async fn safe_copy(src: &Path, dst: &Path) -> io::Result<()> {
...
mod tests {
    ...
    #[tokio::test]
    async fn safe_rename_dst_when_does_not_exist() {
