...
set(B2_COMPILATION_PROPERTIES link=static threading=multi)
...
if (${CMAKE_SYSTEM_NAME} STREQUAL "Android")
    ...
    # The compilation property address-model only accepts 32_64 as a value for creating a binary if it is a darwin module (Apple Darwin) and 64-bit is needed.
    # If a value of 64 is passed to this property in a darwin module, it won't compile. For a darwin module and 32-bit, it is not needed, as it is the default.
    #
    # In any other system, it should accept 32 or 64, for the specific architecture, according to Boost documentation:
    # "The address-model specifies if 32-bit or 64-bit code should be generated by the compiler. 
    # Whether this feature works depends on the used compiler, its version, how the compiler is configured, and the values of the architecture instruction-set features. 
    # Please see the section called “C++ Compilers” for details." -in section: Builtin features
    # https://www.boost.org/doc/libs/1_64_0/doc/html/bbv2/reference.html#bbv2.reference.tools.compiler.darwin
    # TODO: Test this in a diferent enviroment other than MacOS, to confirm ADDRESS_MODEL values work as expected in other compilers.
    if(${CMAKE_HOST_SYSTEM_NAME} STREQUAL "Darwin" AND ${ADDRESS_MODEL} EQUAL 64)
        set(B2_COMPILATION_PROPERTIES ${B2_COMPILATION_PROPERTIES} address-model=32_64)
    else()
        #In darwin modules (Apple Darwin) passing 32 as the value for this property doesn't seem to create a problem.
        #This was tested using an x86 (32-bit abi) emulator.
        #TODO: Test in device.
        set(B2_COMPILATION_PROPERTIES ${B2_COMPILATION_PROPERTIES} address-model=${ADDRESS_MODEL})
    endif()
