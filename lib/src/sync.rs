...
/// Use this to stop execution at specific code points. Similar to e.g. gdb or Visual Studio break
/// points. Example:
///
/// ```
/// let (pause, controller) = breakpoint::new();
///
/// task::spawn(async move {
///     // do stuff #1
///     pause.hit().await.unwrap();
///     // do stuff #2
///     pause.hit().await.unwrap();
///     // do stuff #3
/// });
///
/// controller.on_hit().await.unwrap(); // Stuff #1 is done after this line
/// controller.release();               // Go on to do stuff #2
/// controller.on_hit().await.unwrap(); // Stuff #2 is done after this line
/// controller.release();               // Go on to do stuff #3
/// ```
#[cfg(test)]
pub(crate) mod break_point {
    use super::uninitialized_watch;
    use std::{
        fmt,
        sync::{
    ...
    pub struct BreakPoint {
        shared: Arc<Shared>,
        notify_tx: uninitialized_watch::Sender<()>,
        notify_rx: uninitialized_watch::Receiver<()>,
    }

    impl BreakPoint {
        /// Returns `Some` when `release` was called, and returns `None` when `BreakPointController`
        /// was dropped.
        #[must_use]
        pub async fn hit(&mut self) -> Option<()> {
            self.shared.hit_count.fetch_add(1, Ordering::Relaxed);
            self.notify_tx.send(()).unwrap_or(());

            // Await release
            while !self.shared.is_released() {
    ...
    // Implemented fmt::Debug mainly to avoid implementing it explicitly for structs that have
    // BreakPoint as a member.
    impl fmt::Debug for BreakPoint {
        fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
            write!(
                f,
                "BreakPoint{{ hit_count:{}, release_count:{} }}",
                self.shared.hit_count.load(Ordering::Relaxed),
                self.shared.release_count.load(Ordering::Relaxed)
            )
        }
    }
    ...
    impl BreakPointController {
        /// Returns `Some(())` when `hit` was called, and returns `None` when the `BreakPoint` is
        /// dropped.
        #[must_use]
        pub async fn on_hit(&mut self) -> Option<()> {
            while self.shared.is_released() {
                if self.notify_rx.changed().await.is_err() {
                    return None;
                }
        ...
        pub fn release(&self) {
            self.shared.release_count.fetch_add(1, Ordering::Relaxed);
            self.notify_tx.send(()).unwrap_or(());
        }
    }
    ...
    impl Shared {
        fn is_released(&self) -> bool {
            self.release_count.load(Ordering::Relaxed) >= self.hit_count.load(Ordering::Relaxed)
        }
    }

    pub fn new() -> (BreakPoint, BreakPointController) {
        let shared = Arc::new(Shared {
            hit_count: AtomicU64::new(0),
            release_count: AtomicU64::new(0),
        });

        let (point_to_controller, controller_from_point) = uninitialized_watch::channel();
        let (controller_to_point, point_from_controller) = uninitialized_watch::channel();

        (
            BreakPoint {
                shared: shared.clone(),
                notify_tx: point_to_controller,
                notify_rx: point_from_controller,
            },
            BreakPointController {
                shared,
                notify_tx: controller_to_point,
                notify_rx: controller_from_point,
            },
        )
    }
}
