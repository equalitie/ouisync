...
async fn local_delete_remote_file() {
    ...
    let mut tx = repo_r.db().begin().await.unwrap();
    write_to_file(
        &mut env.rng,
        &mut tx,
        &mut file,
...
async fn local_truncate_remote_file() {
    ...
    let mut tx = repo_l.db().begin().await.unwrap();
    file.fork(&mut tx, repo_l.get_or_create_local_branch().await.unwrap())
        .await
        .unwrap();
    file.truncate(&mut tx, 0).await.unwrap();
    file.flush(&mut tx).await.unwrap();
...
async fn concurrent_delete_update() {
    ...
    let mut tx = repo_r.db().begin().await.unwrap();
    file.seek(&mut tx, SeekFrom::End(-64)).await.unwrap();
    write_to_file(&mut env.rng, &mut tx, &mut file, 64).await;
    file.flush(&mut tx).await.unwrap();
    tx.commit().await.unwrap();
...
async fn write_to_file(
    ...
    rng.fill(&mut buffer[..]);
    file.write(tx, &buffer).await.unwrap();
}
